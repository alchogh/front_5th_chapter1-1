...
`;class f extends i{template(){const e=d(),t=window.location.pathname||"#/",l=o=>t===`${o}`?"text-blue-600 font-bold":"text-gray-600";return`
  ...
  ${w.map(t=>`
    ...
    `}setEvent(){const e=this.$target.querySelector("#login-form");e&&e.addEventListener("submit",t=>{t.preventDefault();const l=e.querySelector("#username").value.trim();l.length?(u({username:l,email:"",bio:""}),n.navigateTo("/")):alert("아이디를 입력해주세요.")})}}class g extends i{template(){const{user:e}=this.props;return`
    <div>
  <div class="bg-gray-100 min-h-screen flex justify-center">
    <div class="max-w-md w-full">
    ${new f({user:e}).template()}
    ...
    `}setEvent(){(()=>{const t=document.getElementById("profile-form");t&&t.addEventListener("submit",l=>{l.preventDefault();const o=t.querySelector("#username").value.trim(),r=t.querySelector("#email").value.trim(),a=t.querySelector("#bio").value.trim(),x={...JSON.parse(localStorage.getItem("user")),username:o,email:r,bio:a};u(x),alert("프로필이 수정되었습니다"),v()})})()}}const v=()=>{const s=window.location.pathname,e=document.getElementById("root"),t=d();if(t&&s==="/login"){history.pushState({},"","/"),v();return}s==="/"?document.getElementById("root").innerHTML=b(t):s==="/profile"?t?new g(e,{user:t}):alert("로그인이 필요합니다"):s==="/login"?new h(e):document.getElementById("root").innerHTML=m()};class ${constructor(){this.routes={},window.addEventListener("popstate",this.handlePopState.bind(this))}addRoute(e,t){this.routes[e]=t}navigateTo(e){history.pushState(null,"",`/front_5th_chapter1-1/${e}`),this.handleRoute(e)}handlePopState(){this.handleRoute(window.location.pathname)}handleRoute(e){const t="/front_5th_chapter1-1/",l=e.startsWith(t)?e.slice(t.length-1)||"/":e,o=this.routes[l],r=document.getElementById("root"),a=d();if(a&&e==="/login"){this.navigateTo("/");return}if(!a&&e==="/profile"){this.navigateTo("/login");return}if(o)o(a);else if(r){const c=document.getElementById("root");new m(c)}}}const n=new $,E={init:()=>{n.addRoute("/",s=>{const e=document.getElementById("root");new b(e,{user:s})}),n.addRoute("/login",()=>{const s=document.getElementById("root");new h(s)}),n.addRoute("/profile",s=>{const e=document.getElementById("root");new g(e,{user:s})}),n.handleRoute(window.location.pathname),document.addEventListener("click",s=>{var t;const e=s.target.closest("a");if(e&&e.id==="logout"&&(s.preventDefault(),y(),n.navigateTo("/login")),e&&((t=e.getAttribute("href"))!=null&&t.startsWith("/"))){s.preventDefault();const l=e.getAttribute("href");n.navigateTo(l)}})}};export{E as C,m as E,h as L,b as M,g as P,d as g,y as r};
